...
class IndexPage extends Component {
  ...
  render() {
    ...
    if(this.props.loadingInit){
      reactSwipe = <ReactSwipe ref="swiper" className="carousel" swipeOptions={{ speed:300 ,continuous: false, callback: this.swiperCallback}}>
                    {
      ...
      <div className="index-container">
        <Swipe  {...props} >
          <div className="hots-container">
            ...
            <div className="middle-title">钜惠高返</div>
            {goodsTab}
...
function mapDispatchToProps(dispatch) {
    return {
      getInitData: function(act){
        dispatch({type:"indexpage/getGoodsInitData"});
      },
      setSwiperActive: function(act){
        dispatch({type:"indexpage/swiperAct",active:act});
      },
      setTabActive: function(act){
        dispatch({type:"indexpage/tabAct",active:act});
      }
    }
}
export default connect(mapStateToProps, mapDispatchToProps)(CSSModules(IndexPage,styles,{allowMultiple:true}));
